# DSH Event Loop 延迟分位说明与Guide

> 指标来源：**本插件（dsh-runtime-observability）** 暴露 `dsh.runtime.event_loop.delay{quantile}`；Companion metrics `dsh_nodejs_eventloop_delay_*` 来自插件依赖的社区包 `@opentelemetry/instrumentation-runtime-node`。
> （Prometheus 实测）。
> Related：[elu-event-loop-utilization.md](./elu-event-loop-utilization.md)（ELU 饱和度，本文的延迟视角）· [README](../README.md)（插件配置）

---

## 1. Definition

**Event Loop 延迟 = 一次定时器回调没有在预定时刻被准点执行、实际被推迟了多久**。

Node 实现：`setTimeout(fn, 1)` 后测 `fn` 真正执行时刻与预定时刻的差。事件循环空闲时差值 ≈ 1ms + 抖动；线程被前一轮任务占住时，差值 = 被占住的时长——所以它直接度量**主线程单次阻塞了多久**。
...
## 5. Diagnosis Example（example snapshot）

| 实例 | p50 / p90 / p99 | 社区包 mean / 1h 峰值 | 判读 |
|---|---|---|---|
| dsh-agent | 20ms / 21ms / 42ms | 10.6ms / 33ms | ✅ 健康 |
| dsh-agent-web2 | 20ms / 25ms / **2.95s** | 94ms / **3.34s** | 🔴 周期性长阻塞（持续恶化：p99 由昨 2.87s → 2.95s） |
...
## 7. Common Queries（Prometheus，已实测）
