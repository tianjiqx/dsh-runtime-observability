...
> 指标来源：**本插件** 暴露 `dsh.telemetry.export.*` 和 `dsh.telemetry.degradation.*`；旧 `dsh.telemetry.export{outcome}` 仅为看板兼容保留
> 关联文档：[ELU 指南](./elu-event-loop-utilization.md)（ELU 是导出暂停的条件之一）· [README](../README.md) 韧性章节（节流/断路器配置）
...
关键区分：**本指标讲的是"进程 → Collector"第一跳；Collector 之后断没断（→ Prometheus/ClickHouse）由巡检负责**。两段别混。
...
|---|---|---|---|
| dsh-agent | 398 | 0 | ✅ 全成功 |
| dsh-agent-web2 | 605 | 18（累计） | ✅ 当前健康：24h +128 次失败集中在 6~12h 前（重启窗口），近 1h 增量 0、`consecutive_failure` 无序列 |

数据面还交叉验证了自保机制：失败未伴随 `log_suppressed`/`circuit_open`（阈值未触发），说明当时是间歇失败而非持续断链。

## 5. 处置 Playbook

1. **正在连续失败**：查 `export.consecutive_failures` 和 `circuit.open`，再检查 Collector 端口与网络
