...
> 指标来源：`dsh.runtime.memory{area}` 由**本插件**直接暴露；`dsh_v8js_memory_*` / `dsh_v8js_gc_*` 来自社区包 `@opentelemetry/instrumentation-runtime-node`（依赖引入）
> 关联文档：[事件循环延迟](./event-loop-delay.md)（GC 是 p99 长阻塞的头号嫌疑）· [README](../README.md)
...
3. **短期缓解**：重启目标进程释放堆（**GUI 宿主重启会杀在跑会话，须人工确认窗口**）
