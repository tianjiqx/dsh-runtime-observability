...
const ACTIVE_WORKLOAD_KINDS = [
	"agent_run",
	"llm",
	"tool",
	"mcp"
];
const QUEUE_WORKLOAD_KINDS = [
	"agent_run",
	"llm",
	"tool",
	"mcp"
];
const RECOVERY_WORKLOAD_KINDS = [
	"session",
	"task",
	"ledger"
];
function zeroRecord$1(keys) {
	return Object.fromEntries(keys.map((key) => [key, 0]));
}
function assertKind(kind, keys, field) {
	if (!keys.includes(kind)) throw new RangeError(`unsupported ${field}: ${kind}`);
}
...
*/
var WorkloadMetrics = class {
	active = zeroRecord$1(ACTIVE_WORKLOAD_KINDS);
	queueDepth = zeroRecord$1(QUEUE_WORKLOAD_KINDS);
	queueOldestAgeSeconds = zeroRecord$1(QUEUE_WORKLOAD_KINDS);
	recoveryBacklog = zeroRecord$1(RECOVERY_WORKLOAD_KINDS);
	begin(kind) {
	...
	setQueue(kind, depth, oldestAgeSeconds = 0) {
...
//#endregion
//#region src/subagent-lifecycle.ts
const SUBAGENT_ACTIVATION_STATES = [
	"running",
	"waiting",
	"settled_pending"
];
const SUBAGENT_LIFECYCLE_EVENTS = [
	"started",
	"disposed",
	"orphaned"
];
...
*/
var SubagentLifecycleMetrics = class SubagentLifecycleMetrics {
	clock;
	activations = /* @__PURE__ */ new Map();
	disposedParents = /* @__PURE__ */ new Map();
	events = zeroRecord(SUBAGENT_LIFECYCLE_EVENTS);
	static PARENT_TOMBSTONE_RETENTION_MS = 3e5;
	constructor(clock = Date.now) {
		this.clock = clock;
	}
	activate(id, parentId, status) {
		...
		this.activations.set(id, {
			...
			stateSinceMs: now,
			orphanedAtMs: orphaned ? now : void 0,
			references: 1
		});
	...
	acceptMessage(id, messageId) {
		const activation = this.activations.get(id);
		if (!activation || activation.accepted.has(messageId)) return;
		activation.accepted.add(messageId);
		this.reconcile(this.clock());
	}
	releaseMessage(id, messageId) {
		if (!this.activations.get(id)?.accepted.delete(messageId)) return;
		this.reconcile(this.clock());
	}
	markParentDisposed(parentId) {
		const now = this.clock();
		this.pruneParentTombstones(now);
		if (this.disposedParents.has(parentId)) return;
		this.disposedParents.set(parentId, now);
		for (const activation of this.activations.values()) if (activation.parentId === parentId && activation.orphanedAtMs === void 0) {
	...
	snapshot() {
		...
		for (const activation of this.activations.values()) {
			...
			oldestAgeSeconds[activation.state] = Math.max(oldestAgeSeconds[activation.state], (now - activation.stateSinceMs) / 1e3);
			if (activation.orphanedAtMs !== void 0) {
				orphans += 1;
				orphanOldestAgeSeconds = Math.max(orphanOldestAgeSeconds, (now - activation.orphanedAtMs) / 1e3);
			}
		}
		return {
			activations,
	...
	reconcile(now) {
		const parentsWithChildren = /* @__PURE__ */ new Set();
		for (const activation of this.activations.values()) parentsWithChildren.add(activation.parentId);
		for (const activation of this.activations.values()) {
			let next;
			if (activation.status === "running" || activation.accepted.size > 0) next = "running";
			else if (parentsWithChildren.has(activation.id)) next = "waiting";
			else next = "settled_pending";
	...
	pruneParentTombstones(now) {
		const cutoff = now - SubagentLifecycleMetrics.PARENT_TOMBSTONE_RETENTION_MS;
		for (const [parentId, disposedAt] of this.disposedParents) if (disposedAt < cutoff) this.disposedParents.delete(parentId);
	}
};
/** Attach to public DSH lifecycle seams without taking a package dependency on DSH internals. */
function installSubagentLifecycleMetrics(ctx, metrics) {
	const root = ctx;
	root.on("agent/disposed", (payload) => {
	...
	root.inject(["subagents"], (injected) => {
		injected.subagents?.registerContinuableSetup((childCtx) => {
			...
			if (!agent || parentId === void 0) return;
			const id = String(agent.id);
			metrics.activate(id, String(parentId), agent.status);
			childCtx.on("agent/status", ({ status }) => metrics.setStatus(id, status));
			childCtx.on("agent/inbox/inserted", ({ message }) => {
				metrics.acceptMessage(id, String(message.id));
			});
			childCtx.on("agent/inbox/claimed", ({ message }) => {
				metrics.releaseMessage(id, String(message.id));
			});
			childCtx.on("agent/inbox/discarded", ({ message }) => {
				metrics.releaseMessage(id, String(message.id));
			});
...
var RuntimeObservability = class {
	workload = new WorkloadMetrics();
	subagents = new SubagentLifecycleMetrics();
	...
	registerMetrics(eluPauseThreshold) {
		...
		meter.addBatchObservableCallback((result) => {
			...
			for (const kind of RECOVERY_WORKLOAD_KINDS) result.observe(workloadRecoveryBacklog, workload.recoveryBacklog[kind], { kind });
			...
			for (const event of SUBAGENT_LIFECYCLE_EVENTS) result.observe(subagentLifecycleEvents, subagents.events[event], { event });
		}, [
			runtime,
			utilization,
			memory,
			...
			workloadActive,
			workloadQueueDepth,
			workloadQueueOldestAge,
			workloadRecoveryBacklog,
			subagentActivations,
			subagentActivationOldestAge,
			subagentOrphans,
			subagentOrphanOldestAge,
...
//#endregion
export { RuntimeObservability, SubagentLifecycleMetrics, WorkloadMetrics, apply, normalizeMetricsEndpoint };
